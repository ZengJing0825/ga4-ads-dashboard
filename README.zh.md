...
**对账不是可选项。** 实际项目里遇到过后端上报缺少会话参数，GA4 比数据库少 10 到 15%，投放算法因此「没有学习对象」。校验层的两件事，文件是否新鲜完整、GA4 事件数和自有数据库是否吻合，就是为堵这个坑。
...
**承接页对照：我最核心的实验。** 看板按承接页类型拆 campaign，因为我的核心实验就是承接方式的对照。结论：单内容页非常吃内容质量，同一预算下效果能差 50% 到 100%；把一个场景下的内容聚合到一个页面去投，campaign 更稳定，成本也最低。我原始看板 30 天快照的数字：

| 承接方式 | 注册成本 | 点击率 |
|---|---|---|
| 场景聚合页系列 | 7 到 9 美元 | 约为 PMax 的 2.3 倍 |
| Performance Max（同期） | 19 到 21 美元 | 基准 |
| 整体混合 CPA | 约 15 美元 | |
